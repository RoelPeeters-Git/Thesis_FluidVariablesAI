...
This file packs all necessary functions to implement training and utilization
of a neural network, it is to be imported in a code where training and test
examples will be run through to train the network and obtain the parameters.
This code is based on the neural network designed in the Coursera course:
...
def initialize_parameters(layer_dims):
    ...
    Input:  layer_dims --   amount of neurons in each layer, len(layer_dims) =
                            # of layers in the NN
    Output: parameters --   a dictionary with all the generated parameters W
                            and b for the linear part of a neuron
    """
    parameters = {}
    L = len(layer_dims)            # number of layers in the network

    for l in range(1, L):
        parameters['W' + str(l)] = np.random.rand(layer_dims[l],
                                                  layer_dims[l-1])
...
def initialize_parameters_Adam(parameters):
    """
    Initializes the extra parameters v and s for use in the Adam optimization
    algorithm
...
def linear_forward(A, W, b):
    ...
    Output:     Z -- linear part of the forward propagation
                cache -- contains current parameters to be used in the
...
def linear_activation_forward(A_prev, W, b, activation):
    ...
    Output:     A -- activation for the current layer of the NN
                cache -- contains all the parameters to be kept for backward
                propagation
    """
...
def L_model_forward(X, parameters, activation):
    ...
    """
    caches = []  # store all the cached parameters of the forward propagation
    A = X
...
def linear_backward(dZ, cache):
    ...
    Output:     dA_prev, dW, db -- gradient of cost with regards to A_prev,
...
def linear_activation_backward(dA, cache, activation):
    ...
    Input:      dA -- gradient of the cost to the current layer's activation
                cache -- current layer's parameters from the forward
                        propagation
                activation -- the activation function used in the current layer
    Output:     dA_prev -- gradient of the cost to the previous layer's
                activation part
...
def L_model_backward(AL, Y, caches, activation):
    ...
    Output:     grads -- dictionary with all the gradients to the cost of the
                        parameter
    """
    grads = {}
...
def update_parameters(parameters, grads, learning_rate):
    ...
    Input:      parameters -- current parameters of the layer
                ...
                learning_rate -- the given learning rate to update the
                                parameters
...
def update_parameters_Adam(parameters, v, s, t, grads, learning_rate):
    ...
    Input:      parameters, v, s -- current parameters of the layer
                ...
                learning_rate --    the given learning rate to update the
                                    parameters
    Output:     parameters, v, s -- updated parameters W and b and v,s for the
                                    current layer
    """
    ...
    for l in range(1, L):
        ...
        parameters[f"W{l}"] = (parameters[f"W{l}"] -
                               learning_rate*v_corr[f"dW{l}"] /
                               (np.sqrt(s_corr[f"dW{l}"])+epsilon))
        parameters[f"b{l}"] = (parameters[f"b{l}"] -
                               learning_rate*v_corr[f"db{l}"] /
                               (np.sqrt(s_corr[f"db{l}"])+epsilon))

    return parameters, v, s


# =====FULL IMPLEMENTATION FOR TRAINING A NEURAL NETWORK =====

def L_layer_model(X, Y, layers_dims, activation, learning_rate=0.0075,
                  num_iterations=3000, print_cost=False, optimizer='GD'):
    """
    ...
    Input:      X -- input features of the training examples
                ...
                optimizer -- optimizer to be used for the update of the
                            parameters
    ...
    # Factor used in printing the cost, depending on the number of iterations
    ...
    for i in range(0, num_iterations):
        ...
        if optimizer == 'GD':
            ...
            t += 1
            parameters, v, s = update_parameters_Adam(parameters, v, s, t,
                                                      grads, learning_rate)
    ...
    return parameters, grads, costs, AL


# Implementing a function to make predictions with the trained NN
def predict_test(X, y, parameters, activation):
    """
    This function is used to predict the results of the L-layer neural network.
    A comparison with the correct results is made, the accuracy is determined.
    Input:  X -- the inputs for which a prediction is to be made
            y -- the true output for comparison
            parameters -- the NN parameters
            activation -- the activation to be used
    Output: p -- the predictions
            error -- the absolute error
            rel_error -- the RMS error
    """
    ...
    error = np.abs(p-y)
    rel_error = np.sqrt(np.mean(error**2)/np.mean(y**2))
...
# # =====gradient checking function of a parameter calculated in the NN =====
